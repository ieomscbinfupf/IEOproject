...
subtitle: "IEO 2026, MSc Bioinformatics, UPF"
author:
- name: "Robert Castelo"
...
abstract: >
  ...
  respiratory viruses, namely SARS-CoV-2, seasonal influenza A virus,
  and the human respiratory syncytial virus. We find that library
  preparation protocols differed among cell lines, precluding comparing
  them, but found differentially expressed genes due to infection
...
expression (see the help page of the `filterByExpr()` function in
the `r Biocpkg("edgeR")` package). We should keep an eye on samples
with these biases in case they also display other unexpected features,
...
Here we either write that we did not use any AI tool to help with the analysis
or, in case we have used it, we need to include here the following information:
